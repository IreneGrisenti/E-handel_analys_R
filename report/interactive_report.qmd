...
## Returgrad mellan olika kategorier
...
## Returgrad mellan olika kundtyper
...
## Returfrekvensen baserat på olika leveransgrupper
...
## Kontroll av genomsnittlig leveranstid för returnerade och ej returnerade ordrar
...
Analysen visar flera tydliga mönster i returdata som kan ge företaget ett bättre beslutsunderlag. Den mest användbara insikten är sambandet mellan leveranstid och returgrad: ordrar med längre leveranstid returneras i betydligt högre utsträckning (20,8% för Slow jämfört med 9,9% för Fast), vilket tyder på att förbättrad leveranseffektivitet direkt kan minska antalet returer.

Även kundtyp visade sig vara en viktig faktor, där nya kunder returnerar varor oftare (18,0%) än återkommande och VIP-kunder. En begränsning här är att vi inte vet varför de returnerar, och att ett tillägg av returorsaker i datasetet skulle kunna berika kvaliteten på returanalysen.

Gällande produktkategorier hade Fashion och Home högst returgrad, medan Sports hade lägst, vilket kan användas för att rikta förbättringsåtgärder mot specifika kategorier. Här vore det intressant att undersöka om leveranstiden påverkar returgraden inom respektive kategori.

Rabattanalysen visade en tydlig ökning av returgrad med högre rabatter, men resultatet bör tolkas med försiktighet på grund av den kraftiga snedfördelningen i den högsta rabattgruppen, som endast representerades av 4 ordrar. Här skulle det vara värdefullt att samla in mer data eller se över indelningen av rabattnivåerna.

Datasetet består av endast 1000 ordrar, vilket begränsar möjligheten att dra generella slutsatser, särskilt för mindre grupper som High-rabatt och VIP-kunder. Analysen tar inte heller hänsyn till om returmönster förändrats över tid, exempelvis till följd av säsongsvariationer kring jul eller reor. Slutligen kan de saknade värden som fylldes i med median eller typvärde ha påverkat resultaten något, även om påverkan bedöms vara begränsad.

# Kompletterande analys

## Andel per kundsegment
...
## Genomsnittligt ordervärde per kundsegment
...
## Kombination av segment och kundtyp med returgrad
...
-   *Kundsegment:* `Consumer` utgjorde den största gruppen (52,9%), följt av `Small Business` (30,2%) och slutligen `Corporate` (16,9%).
